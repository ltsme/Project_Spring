...
public class BAdoptcommand implements BCommand_new {
	@Override
	public void execute(SqlSession sqlSession, Model model,HttpSession httpSession) {
		// TODO Auto-generated method stub
    
		Map<String, Object> map = model.asMap(); 
		HttpServletRequest request = (HttpServletRequest) map.get("adoptwait");
    ...
    int startRow = (currentPage - 1) * pageSize ; //해당페이지에서 시작할 게시물
    int endRow = pageSize;              // 해당페이지 마지막 게시물
 
    
    //iDao dao = sqlSession.getMapper(iDao.class);
	//model.addAttribute("list", dao.listDao());
    System.out.println("시작 게시물"+startRow);
    ...
    ArrayList<BDtoAdopt> dtos = daoadopt.adoptwait(startRow, endRow); // 해당 페이지에 알맞은 번호의 게시글
         ...
         System.out.println("ssssssssssssssssssssss"+adoptst);
